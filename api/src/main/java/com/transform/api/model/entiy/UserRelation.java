...
public class UserRelation implements Serializable {
    ...
    //操作人
    @Column(name = "uuidUserA", nullable = false, length = 64)
    private String uuidUserA;

    //被执行人
    @Column(name = "uuidUserB", nullable = false, length = 64)
    private String uuidUserB;

    //关系状态，follow关注为1，unfriend屏蔽为2，block拉黑为3，无关系则直接删除记录
    @Column(name = "relationStatus", nullable = false, length = 64)
    private Integer relationStatus;

    @Column(name = "operTime", nullable = false, length = 64)
